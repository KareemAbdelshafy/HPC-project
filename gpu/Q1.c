...
__host__void init_vects(int vect_len,float *h_vect1,float *h_vect2);
__global__void vec_add(int vect_len, float *v1, float *v2, float *d_sum);
...
__global__void vec_add(int vect_len, float *d_vect1, float *d_vect2, float *d_sum){

int tid = blockIdx.x * blockDim.x + threadIdx.x ;

if(tid<vect_len) d_sum[tid]= d_vect1[tid] + d_vect2[tid];

}


__host__void init_vects(int vect_len,float *vect1,float *vect2){
srand(time(NULL));
